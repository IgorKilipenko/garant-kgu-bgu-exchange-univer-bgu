...
Процедура ПолучитьДанныеУниверситета(Команда)
    РезультатПолученияДанных = ПолучитьСырыеДанныеУниверситетНаСервере();
...
Функция ПолучитьСырыеДанныеУниверситетНаСервере()
    ...
    //     ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуДатыJSON(ЭтотОбъект.Объект.ПериодОбработки.ДатаНачала));
    // Сообщение.Параметры.Вставить("ДатаОкончания",
    //     ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуДатыJSON(ЭтотОбъект.Объект.ПериодОбработки.ДатаОкончания));
    // Сообщение.Параметры.Вставить("НомерПриказа", ЭтотОбъект.Объект.НомерПриказа);
    // Сообщение.Параметры.Вставить("ТолькоПлатнаяОснова",
    //     ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуИзБулево(Истина));
    //
    ...
    //
    // // Получение сообщений ответа
    // ПараметрыОтбора = ГП_ОбменУниверситетБГУ.НовыйПараметрыОтбораСообщенийОтветаУниверситета(
    //     РезультатФункции.ИдентификаторСообщенияЗапроса);

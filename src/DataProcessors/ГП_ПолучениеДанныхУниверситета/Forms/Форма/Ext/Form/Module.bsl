...
Процедура ПолучениеДанныхУниверситетаЗавершение(Знач РезультатВыполнения, Знач ДополнительныеПараметры = Неопределено) Экспорт
    ...
    ЗаполнитьДанныеДляЗагрузкиНаСервере(СтруктураТелаОтвета.Данные);

    Состояние("Выполнено получение данных университета");
...
Функция ЗаполнитьДанныеДляЗагрузкиНаСервере(Знач ДанныеЗагрузки)
    ...
    Для Каждого СтрокаДанныхПриказов Из ДанныеЗагрузки.Приказы Цикл
        НоваяСтрокаПриказа = ЭтотОбъект.Объект.Приказы.Добавить();
        ЗаполнитьЗначенияСвойств(НоваяСтрокаПриказа, СтрокаДанныхПриказов);

        НоваяСтрокаПриказа.Организация = ОрганизацияПоУмолчанию;
        НоваяСтрокаПриказа.НомерУниверситет = СтрокаДанныхПриказов.НомерПриказа;
        НоваяСтрокаПриказа.ИдентификаторУниверситет = СтрокаДанныхПриказов.УникальныйИдентификатор;
        НоваяСтрокаПриказа.ПредставлениеУниверситет = СтрокаДанныхПриказов.ПредставлениеСсылки;
    КонецЦикла;
...
Процедура ОчиститьДанныеЗагрузкиНаСервере()
    ЭтотОбъект.Объект.Приказы.Очистить();
КонецПроцедуры

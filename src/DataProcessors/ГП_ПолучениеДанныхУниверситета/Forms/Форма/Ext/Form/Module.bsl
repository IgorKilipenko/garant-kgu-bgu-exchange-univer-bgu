...
&НаСервере
Функция ПолучитьСырыеДанныеУниверситетНаСервере(Знач КодПолучателяУниверситет = Неопределено)
    КодПолучателяУниверситет = ?(ПустаяСтрока(КодПолучателяУниверситет), ПолучитьКодПолучателяУниверситет(), КодПолучателяУниверситет);

    Сообщение = СервисыИнтеграции.ГП_СервисИнтеграцииБГУУниверситет.СоздатьСообщение();
    Сообщение.КодПолучателя = КодПолучателяУниверситет;

    Сообщение.Параметры.Вставить("ТипСообщения", "ЗапросДанныхПриказа");
    СервисыИнтеграции.ГП_СервисИнтеграцииБГУУниверситет.ОбменУниверситет_ИзБухгалтерии.ОтправитьСообщение(Сообщение);

    Возврат Неопределено; //СервисыИнтеграции.ГП_СервисИнтеграцииБГУУниверситет.ОбменУниверситет_ИзБухгалтерии.ПолучитьСостояние();
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьКодПолучателяУниверситет()
    Возврат ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьКодПолучателяУниверситет();
КонецФункции

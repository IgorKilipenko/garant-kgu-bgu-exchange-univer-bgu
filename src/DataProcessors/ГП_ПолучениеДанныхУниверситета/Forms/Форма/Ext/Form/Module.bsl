...
Функция ПолучитьСырыеДанныеУниверситетНаСервере()
    РезультатФункции = Новый Структура("Успех, МассивСообщений", Истина, Новый Массив);
    
    РаботаВТестовомРежиме = РаботаВТестовомРежиме(ЭтотОбъект);
    
    КодБазыУниверситет =  ПолучитьКодБазыУниверситет(РаботаВТестовомРежиме);
    КодБазыБГУ =  ПолучитьКодБазыБГУ(РаботаВТестовомРежиме);
    ...
    ПараметрыОтбора.Вставить("ИдентификаторСообщенияЗапроса", УникальныйИдентификаторСообщенияЗапроса);
    ...
    РезультатФункции.МассивСообщений = ГП_ОбменУниверситетБГУ.ПолучитьСообщенияСервиса(
        КаналОбменаИзУниверситета, ПараметрыОтбора, 1000);
...
Функция ПолучитьКодБазыУниверситет(Знач Тестовый = Ложь)
    Возврат ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьКодБазыУниверситет(Тестовый);
КонецФункции

&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьКодБазыБГУ(Знач Тестовый = Ложь)

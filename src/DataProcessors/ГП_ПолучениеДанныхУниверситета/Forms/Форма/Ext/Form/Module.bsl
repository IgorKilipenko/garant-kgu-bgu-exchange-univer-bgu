...
Процедура ПолучитьДанныеУниверситета(Команда)
    ...
    Если РезультатПолученияДанных.Успех = Ложь ИЛИ РезультатПолученияДанных.МассивСообщений.Количество() = 0 Тогда
        ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(
                "Не удалось получить данные от базы << %1 >>.
                |Информация: %2",
                ПолучитьПредставлениеНаименованияБазыУниверситет(),
                РезультатПолученияДанных.ТекстСообщения));

        Возврат; // Ошибка получения данных университет
    КонецЕсли;

    МассивОтветовУниверситет = РезультатПолученияДанных.МассивСообщений;
    Если МассивОтветовУниверситет.Количество() > 1 Тогда
        ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон(
                "ВНИМАНИЕ! Неоднозначный отвеет от базы << %1 >>", ПолучитьПредставлениеНаименованияБазыУниверситет()));
    КонецЕсли;

    ОтветУниверситета = МассивОтветовУниверситет[0];
    ...
    Иначе
        // TODO
        ОбщегоНазначенияКлиент.СообщитьПользователю("Данные получены");
    КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьПроверкуСоединенияВФоне(Команда)
    ВыполнитьОперациюПроверкуСоединения();
КонецПроцедуры
...
&НаКлиенте
Процедура ВыполнитьОперациюПроверкуСоединения()
...
Процедура ПроверкаСтатусаПодключенияЗавершение(Знач РезультатВыполнения, Знач ДополнительныеПараметры = Неопределено) Экспорт
    ...
    Если РезультатВыполнения = Неопределено
        ИЛИ РезультатВыполнения.Статус = "Ошибка"

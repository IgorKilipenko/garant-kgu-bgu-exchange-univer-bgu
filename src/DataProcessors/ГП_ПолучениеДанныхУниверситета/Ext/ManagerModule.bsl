...
Функция ЗаполнитьДанныеИзУниверситет(Объект, Знач ДанныеЗагрузки, Знач ИдентификаторХранилища = Неопределено) Экспорт
    ...
    Для Каждого СтрокаДанныхПриказов Из ДанныеЗагрузки.Приказы Цикл
        Для Каждого ИдентификаторФакультета Из СтрокаДанныхПриказов.Факультеты Цикл
            ...
            НоваяСтрокаПриказа.Комментарий = СтрокаДанныхПриказов.Комментарий;
    ...
    Для Каждого ТаблицаДанныхКЗ Из ДанныеЗагрузки.СвязанныеОбъекты Цикл
        ...
        Если ЭтоПредопределеннаяТаблица Тогда
            ...
            ИначеЕсли ТаблицаДанныхКЗ.Ключ = ИнформациюОПредопределенныхТаблиц.Специальности.Наименование Тогда
                НоваяТаблицаДанных = ГП_ОбменУниверситетБГУОбработкаДанныхУниверситет.НовыйТаблицаСпециальностейОбучения();
            КонецЕсли;
...
Функция СоздатьОбъектыОбменаПолученныеИзУниверситет(Объект) Экспорт
    РезультатФункции = Новый Структура(
            "Успех, ТекстСообщения, СообщенияОбОшибках, ИнформационныеСообщения",
            Истина, Неопределено, Новый Массив, Новый Массив);
    ...
    /////////////////////////////////////////////
    ОтменитьТранзакциюПринудительно = Ложь; // !!! Установить Истина - при тестировании
    ...
    ТаблицаСпециальностей = КоллекцияТаблицСвязанныхОбъектов.Специальности;
    ТаблицаФормОбучения = КоллекцияТаблицСвязанныхОбъектов.ФормыОбучения;
    ТаблицаИдентификаторовОбщихДанныхПриказа = КоллекцияТаблицСвязанныхОбъектов.ТаблицаИдентификаторовОбщихДанныхПриказа;
    ...
    Попытка
        ...
        // Сопоставление (и создание) специальностей обучения
        РезультатСопоставленияСпециальностей =
            ГП_ОбменУниверситетБГУОбработкаДанныхУниверситет.СопоставитьСпециальностиПолученныеИзУниверситет(
                ТаблицаСпециальностей, Истина, ДанныеТранзакции);
        Если РезультатСопоставленияСпециальностей.Успех = Ложь Тогда
            ВызватьИсключение(РезультатСопоставленияСпециальностей.ТекстСообщения); // Отмена транзакции по ошибке сопоставления
        КонецЕсли;
        ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
            РезультатФункции.СообщенияОбОшибках, РезультатСопоставленияСпециальностей.СообщенияОбОшибках);
        ...
        КэшДанныхПриказовСтудентов = ГП_ОбменУниверситетБГУОбработкаДанныхУниверситет.ПолучитьСтрокиТаблицыПриказовДляСтудентовУниверситет(
                ТаблицаСтудентов, ТаблицаПриказов, ТаблицаИдентификаторовОбщихДанныхПриказа);
        ...
        ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
            РезультатФункции.ИнформационныеСообщения, РезультатСопоставленияДоговоров.ИнформационныеСообщения);

        // Сопоставление (и создание) Приказов
        ОбщаяТаблицаДанных = Новый Структура;
        ОбщаяТаблицаДанных.Вставить("ТаблицыСвязанныхОбъектов", Новый Структура);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаФакультетов", ТаблицаФакультетов);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаСтудентов", ТаблицаСтудентов);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаДоговоров", ТаблицаДоговоров);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаТиповПриказов", ТаблицаТиповПриказов);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаКурсовОбучения", ТаблицаКурсовОбучения);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаСпециальностей", ТаблицаСпециальностей);
        ОбщаяТаблицаДанных.ТаблицыСвязанныхОбъектов.Вставить("ТаблицаФормОбучения", ТаблицаФормОбучения);
        ОбщаяТаблицаДанных.Вставить("ТаблицаИдентификаторовОбщихДанныхПриказа", ТаблицаИдентификаторовОбщихДанныхПриказа);
        РезультатСопоставленияПриказов =
            ГП_ОбменУниверситетБГУОбработкаДанныхУниверситет.СопоставитьПриказыОбученияПолученныеИзУниверситет(
                ТаблицаПриказов,
                ОбщаяТаблицаДанных,
                Истина,
                ДанныеТранзакции);
        Если РезультатСопоставленияПриказов.Успех = Ложь Тогда
            ВызватьИсключение(РезультатСопоставленияПриказов.ТекстСообщения); // Отмена транзакции по ошибке сопоставления
        КонецЕсли;
        ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
            РезультатФункции.СообщенияОбОшибках, РезультатСопоставленияПриказов.СообщенияОбОшибках);
        ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
            РезультатФункции.ИнформационныеСообщения, РезультатСопоставленияПриказов.ИнформационныеСообщения);

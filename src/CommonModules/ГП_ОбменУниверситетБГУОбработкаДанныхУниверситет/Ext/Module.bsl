...
// Параметры:
//  ТаблицаСтудентов - ТаблицаЗначений - Изменяется в процессе выполнения функции
//          (заполняется Договор Студент, ИдентификаторПриказа, ИдентификаторФакультета)
//          Таблица заполняется в процессе выполнения функции
...
Функция СопоставитьДоговорыСтудентовПолученныеИзУниверситет(
    ...
    РезультатФункции.Вставить("СоответствиеСозданныхОбъектов", Новый Соответствие);
    РезультатФункции.Вставить("СообщенияОбОшибках", Новый Массив);
    РезультатФункции.Вставить("ИнформационныеСообщения", Новый Массив);
    ...
    Для Каждого СтрокаТаблицы Из ТаблицаСтудентов Цикл
        ...
        Для Каждого СтрокаПриказа Из СтрокиПриказовСтудента Цикл
            ...
            Если ЗначениеЗаполнено(СтрокаТаблицы.Студент) И ОбщегоНазначения.СсылкаСуществует(СтрокаТаблицы.Студент) Тогда
                ...
                СтруктураДанныхДоговораДляОтбора = Новый Структура(
                        "Идентификатор, ИдентификаторПриказа, ИдентификаторФакультета, ДатаДоговора",
                        СтрокаТаблицы.Идентификатор,
                        СтрокаПриказа.Идентификатор,
                        СтрокаПриказа.ИдентификаторФакультета,
                        МассивДатОтбора);
                ...
                Если СтруктураНайденногоДоговора <> Неопределено Тогда
                    ...
                    РезультатФункции.ИнформационныеСообщения.Добавить(СтрШаблон(
                            "Внимание! Договор со Студентом: << %1, ид: %2 >> Приказа: << %3 от %4, ид: %5 >> Факультета: << %6, ид: %7 >>"
                            + " не был создан т.к. был найден ранее созданный справочник договора с соответствующими"
                            + " значениями реквизитов документа (установлено соответствие обмена).",
                            СтрокаТаблицы.Наименование,
                            СтрокаТаблицы.Идентификатор,
                            СтрокаПриказа.НомерПриказа,
                            Формат(НачалоДня(ДатаДоговора), "ДФ='dd.MM.yyyy г.'"),
                            СтрокаПриказа.Идентификатор,
                            СтрокаПриказа.НаименованиеФакультета,
                            СтрокаПриказа.ИдентификаторФакультета
                        ));
                ИначеЕсли СоздаватьНовыеЕслиОтсутствует Тогда
                    // Создание нового вида приказа
                    ...
                    ЗаписатьСоответствияИдентификаторовОбмена(
                        ...
                        // !!! Т.к. договор не переносится из Университет - в качестве идентификатора используется ид студента
                        СтрокаТаблицы.Идентификатор,
                        ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.Договор,
                        ДанныеТранзакции.Идентификатор,
                        ДанныеТранзакции.Дата,
                        ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.Приказ,
                        СтрокаПриказа.Идентификатор);
...
// Выполняет сопоставление загруженных приказов базы Университет с приказами базы БГУ
//  В случе отсутствия приказа в базу БГУ - создает новый
// Параметры:
//  ТаблицаПриказов - ТаблицаЗначений
//  ОбщаяТаблицаДанных - Структура из КлючИЗначение
//      * Ключ - Строка
//      * Значение - ТаблицаЗначений, Структура
//  СоздаватьНовыеЕслиОтсутствует - Булево
//  ДанныеТранзакции - Структура
//      * Идентификатор - Строка
//      * Дата - Дата
...
Функция СопоставитьПриказыОбученияПолученныеИзУниверситет(
    ...
    Для Каждого СтрокаПриказа Из ТаблицаПриказов Цикл // Заполнение приказов (по факультетам)
        ...
        Если ЗначениеЗаполнено(ДанныеФакультетаПриказа.Факультет) = Ложь
            ...
            СтрокаПриказа.Приказ = Документы.ПриказОбразование.ПустаяСсылка(); // Очистка поля связи
            Продолжить;
        КонецЕсли;
        ...
        // Заполнение основных реквизитов приказа
        ДанныеРеквизитовПриказа = Новый Структура;
        ДанныеРеквизитовПриказа.Вставить("Номер", СтрШаблон("%1/%2", Строка(ПрефиксПриказа), СтрокаПриказа.НомерПриказа));
        ДанныеРеквизитовПриказа.Вставить("Дата", ДатаПриказа);
        ДанныеРеквизитовПриказа.Вставить("ДатаНачалаДействия", СтрокаПриказа.КанцелярскаяДата);
        ДанныеРеквизитовПриказа.Вставить("Организация", Организация);
        ДанныеРеквизитовПриказа.Вставить("Факультет", ДанныеФакультетаПриказа.Факультет);
        ДанныеРеквизитовПриказа.Вставить("ВидПриказа", ДанныеВидаПриказа.ВидПриказа);
        // Заполнение поля Комментарий
        ДанныеРеквизитовПриказа.Вставить("Комментарий", СтрШаблон("%1/%2", СтрокаПриказа.НомерПриказа, СтрокаПриказа.Комментарий));

        // Создание документа
        НовыйПриказОбразованияОбъект = Документы.ПриказОбразование.СоздатьДокумент();
        ЗаполнитьЗначенияСвойств(НовыйПриказОбразованияОбъект, ДанныеРеквизитовПриказа);
        ЭтоНовыйДокументПриказа = Истина;

        // Проверка наличия ранее созданного приказа
        ДанныеРеквизитовПриказа.Вставить("НомерПриказаБГУ", ДанныеРеквизитовПриказа.Номер);
        РезультатПоискаПриказа = НайтиПриказОбразованияПоДаннымУниверситет(СтрокаПриказа, ДанныеРеквизитовПриказа);
        Если РезультатПоискаПриказа <> Неопределено Тогда
            СтрокаПриказа.Приказ = РезультатПоискаПриказа.Приказ;
            ЭтоНовыйДокументПриказа = Ложь;
            РезультатФункции.ИнформационныеСообщения.Добавить(СтрШаблон(
                    "Внимание! Приказ: << № %1 от %2 >> (Факультет: << %3 >> Строка: << %4 >>) не был создан т.к."
                    + " был найден ранее созданный документ приказа с соответствующими"
                    + " значениями реквизитов документа (установлено соответствие обмена).",
                    СтрокаПриказа.НомерПриказа,
                    Формат(ДатаПриказа, "ДФ='dd.MM.yyyy г."),
                    ДанныеФакультетаПриказа.Наименование,
                    СтрокаПриказа.НомерСтроки
                ));
        КонецЕсли;

        Если ЭтоНовыйДокументПриказа = Ложь И ОбновлятьДанныеПриказа = Ложь Тогда
            Продолжить; // Документ создан ранее и нет обновлений
        КонецЕсли;

        // Кэш данных строк ТЧ приказа
        КэшДетальныхДанныхПриказа = Новый Структура;
        КэшДетальныхДанныхПриказа.Вставить("Студенты", Новый Соответствие);
        КэшДетальныхДанныхПриказа.Вставить("КурсыОбучения", Новый Соответствие);
        КэшДетальныхДанныхПриказа.Вставить("Специальности", Новый Соответствие);
        КэшДетальныхДанныхПриказа.Вставить("ФормыОбучения", Новый Соответствие);
        ...
        Для Каждого СтрокаИдентификаторовСвязей Из НайденныеСтрокиИдентификаторовФакультетовПриказа Цикл
            ...
            Если ДанныеСтудентаПриказа.Количество() = 0 Тогда
                ...
                ДанныеСтудентаПриказа = Неопределено;
                РезультатФункции.Успех = Ложь;
                РезультатФункции.ТекстСообщения = "Ошибка заполнения данных приказов"; // Продолжение только для сбора ошибок
            ...
            Если ЗначениеЗаполнено(ДанныеСтудентаПриказа.Студент) = Ложь // Проверка студента
                ...
                КэшДетальныхДанныхПриказа.Студенты.Вставить(
                    СтрокаИдентификаторовСвязей.ИдентификаторСтудента, ДанныеСтудентаПриказа);
                Продолжить;
            КонецЕсли;
            ...
            ДанныеКурсаОбучения = КэшДетальныхДанныхПриказа.КурсыОбучения.Получить(СтрокаИдентификаторовСвязей.ИдентификаторКурсаОбучения);
            Если ДанныеКурсаОбучения = Неопределено Тогда
                ...
                КэшДетальныхДанныхПриказа.КурсыОбучения.Вставить(
                    СтрокаИдентификаторовСвязей.ИдентификаторКурсаОбучения, ДанныеКурсаОбучения);
            КонецЕсли;

            // Заполнение ранее созданной или найденной специальности
            ДанныеСпециальности = КэшДетальныхДанныхПриказа.Специальности.Получить(СтрокаИдентификаторовСвязей.ИдентификаторСпециальности);
            Если ДанныеСпециальности = Неопределено Тогда
                ...
                КэшДетальныхДанныхПриказа.Специальности.Вставить(
                    СтрокаИдентификаторовСвязей.ИдентификаторСпециальности, ДанныеСпециальности);
            КонецЕсли;

            // Заполнение ранее созданной или найденной формы обучения
            ДанныеФормыОбучения = КэшДетальныхДанныхПриказа.ФормыОбучения.Получить(СтрокаИдентификаторовСвязей.ИдентификаторФормыОбучения);
            Если ДанныеФормыОбучения = Неопределено Тогда
                ...
                КэшДетальныхДанныхПриказа.ФормыОбучения.Вставить(
                    СтрокаИдентификаторовСвязей.ИдентификаторФормыОбучения, ДанныеФормыОбучения);
            КонецЕсли;
...
//  ИдентификаторУниверситетДополнительный - Строка
// Возвращаемое значение:
//  - Массив - Ссылка
Функция ПолучитьФильтрСсылокОбъектаИБПоИдентификаторуУниверситет(
        Знач ИдентификаторУниверситет, Знач ВидОбъектаОбмена,
        Знач МенеджерОбъекта, Знач ВидОбъектаОбменаДополнительный = Неопределено, Знач ИдентификаторУниверситетДополнительный = "") Экспорт

    ИдентификаторУниверситетДополнительный = ?(ПустаяСтрока(ИдентификаторУниверситетДополнительный),
            "", ИдентификаторУниверситетДополнительный);

    РезультатФункции = Новый Массив;

    СтруктураОтбора = Новый Структура;
    СтруктураОтбора.Вставить("ИдентификаторВнешний", ИдентификаторУниверситет);
    СтруктураОтбора.Вставить("ИдентификаторВнешнийДополнительный", ИдентификаторУниверситетДополнительный);
    ТаблицаИдентификаторов = ПолучитьАктуальныеДанныеИдентификаторовДляВидаОбъектаОбмена(ВидОбъектаОбмена,
            ВидОбъектаОбменаДополнительный, СтруктураОтбора);
...
Функция НайтиСтудентаПоДаннымУниверситет(
    ...
    Если ТаблицаДанных.Количество() > 0 Тогда
        РезультатФункции = Новый Структура(
                "Студент, Код, Наименование, Контрагент, ФизическоеЛицо, СНИЛС, УникальныйИдентификатор, УстановленаСвязьПоИдентификатору");
        ЗаполнитьЗначенияСвойств(РезультатФункции, ТаблицаДанных[0]);
...
Функция НайтиДоговорОказанияОбразовательныхУслугПоДаннымСтудентаУниверситет(
    ...
    ФильтрСсылок = ПолучитьФильтрСсылокОбъектаИБПоИдентификаторуУниверситет(
            ...
            ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.Приказ,
            СтруктураДанныхДокументаУниверситет.ИдентификаторПриказа);
...
Функция НайтиВидПриказаПоДаннымУниверситет(Знач СтруктураДанныхУниверситет) Экспорт
    ...
    Если ТаблицаДанных.Количество() > 0 Тогда
        РезультатФункции = Новый Структура("ВидПриказа, Код, Наименование, УникальныйИдентификатор, УстановленаСвязьПоИдентификатору");
        ЗаполнитьЗначенияСвойств(РезультатФункции, ТаблицаДанных[0]);
        РезультатФункции.УстановленаСвязьПоИдентификатору = ИспользуетсяОтборПоФильтруСсылок;
    КонецЕсли;
...
// Параметры:
//  СтруктураДанныхУниверситет - Структура, СтрокаТаблицыЗначений
//      * НомерПриказа - Строка - Номер приказа в базе Университет
//      * Идентификатор - Строка
//      * ИдентификаторФакультета - Строка
//  ДополнительныеПараметрыОтбора - Структура
//      * НомерПриказаБГУ - Строка
//      * Факультет - СправочникСсылка.Подразделения
//      * ВидПриказа - СправочникСсылка.ВидыПриказовОбразование
//      * Организация - Организации
// Возвращаемое значение:
//  - Структура, Неопределено
//      * Приказ - ДокументСсылка.ПриказОбразование
//      * Дата - Дата
//      * Номер - Строка
//      * Комментарий - Комментарий
//      * УникальныйИдентификатор - УникальныйИдентификатор
//      * УстановленаСвязьПоИдентификатору - Булево
Функция НайтиПриказОбразованияПоДаннымУниверситет(Знач СтруктураДанныхУниверситет, Знач ДополнительныеПараметрыОтбора) Экспорт
    ВидОбъектаОбмена = ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.ВидПриказа;
    ВидОбъектаОбменаДополнительный = ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.Факультет;
    ФильтрСсылок = ПолучитьФильтрСсылокОбъектаИБПоИдентификаторуУниверситет(
            СтруктураДанныхУниверситет.Идентификатор,
            ВидОбъектаОбмена,
            Документы.ПриказОбразование,
            ВидОбъектаОбменаДополнительный,
            СтруктураДанныхУниверситет.ИдентификаторФакультета);

    Запрос = Новый Запрос;
    Запрос.Текст =
        "ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ПриказыОбразование.Ссылка КАК Приказ,
        |   ПриказыОбразование.Дата КАК Дата,
        |   ПриказыОбразование.Номер КАК Номер,
        |   ВЫРАЗИТЬ(ПриказыОбразование.Комментарий КАК СТРОКА(200)) КАК Комментарий,
        |   ВЫБОР
        |       КОГДА &ИспользуетсяОтборПоФильтруСсылок = ИСТИНА ТОГДА (&МинимальныйПриоритетОтбора)
        |       ИНАЧЕ 0
        |   КОНЕЦ КАК ПриоритетПоФильтруСсылок,
        |   ВЫБОР
        |       КОГДА &ГодПриказа = НачалоПериода(ПриказыОбразование.Дата, ГОД) И &НомерПриказаБГУ <> """" ТОГДА
        |           ВЫБОР
        |               КОГДА ПриказыОбразование.Номер = &НомерПриказаБГУ ТОГДА 3
        |               КОГДА ВЫРАЗИТЬ(ПриказыОбразование.Комментарий КАК СТРОКА(200)) ПОДОБНО &НомерПриказаШаблонКомментария СПЕЦСИМВОЛ ""~""
        |                   ТОГДА (&МинимальныйПриоритетОтбора + 1)
        |               КОГДА ПриказыОбразование.Номер ПОДОБНО &НомерПриказаШаблон СПЕЦСИМВОЛ ""~"" ТОГДА 1
        |               ИНАЧЕ 0
        |           КОНЕЦ
        |       ИНАЧЕ 0
        |   КОНЕЦ КАК ПриоритетПоНомеру
        |ПОМЕСТИТЬ ВТ_ПриказыОбразование_Приоритеты
        |ИЗ
        |   Документ.ПриказОбразование КАК ПриказыОбразование
        |ГДЕ
        |   ИСТИНА
        |   И &УсловияФильтраСсылок
        // |   И ВЫБОР
        // |       КОГДА &ИспользуетсяОтборПоФильтруСсылок = ИСТИНА ТОГДА ИСТИНА
        // |       ИНАЧЕ
        // |           ПриказыОбразование.ПометкаУдаления = ЛОЖЬ
        // |   КОНЕЦ
        |   И НачалоПериода(ПриказыОбразование.Дата, ГОД) = &ГодПриказа
        |   И ПриказыОбразование.Факультет = &Факультет
        |   И ПриказыОбразование.ВидПриказа = &ВидПриказа
        |   И ПриказыОбразование.Организация = &Организация
        |;
        |
        |///////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ПриказыОбразование.*,
        |   ПриказыОбразование.ПриоритетПоФильтруСсылок
        |       + ПриказыОбразование.ПриоритетПоНомеру КАК Приоритет
        |ПОМЕСТИТЬ ВТ_Результат_Подготовка
        |ИЗ
        |   ВТ_ПриказыОбразование_Приоритеты КАК ПриказыОбразование
        |;
        |
        |///////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |   ПриказыОбразование.*,
        |   УникальныйИдентификатор(ПриказыОбразование.Приказ) КАК УникальныйИдентификатор
        |ИЗ
        |   ВТ_Результат_Подготовка КАК ПриказыОбразование
        |ГДЕ
        |   ПриказыОбразование.Приоритет > &МинимальныйПриоритетОтбора
        |
        |УПОРЯДОЧИТЬ ПО
        |   ПриказыОбразование.Приоритет УБЫВ,
        |   ПриказыОбразование.Приказ УБЫВ
        |";

    // НомерПриказаБезСуффикса = СтрЗаменитьПоРегулярномуВыражению(СтруктураДанныхУниверситет.Номер, "^(.*+)/(?:[0-9-A-za-zА-Яа-я]+\s*)$", "$1");
    НомерПриказаУниверситет = СтруктураДанныхУниверситет.НомерПриказа;
    НомерПриказаУниверситетЭкранированный = ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(НомерПриказаУниверситет);
    НомерПриказаШаблонКомментария = СтрШаблон("%1/%%", НомерПриказаУниверситетЭкранированный); // Для поиска в комментарии
    НомерПриказаШаблон = СтрШаблон("%%/%1", НомерПриказаУниверситетЭкранированный); // Для поиска в номере документа

    Запрос.УстановитьПараметр("МинимальныйПриоритетОтбора", 5);
    Запрос.УстановитьПараметр("НомерПриказаШаблон", НомерПриказаШаблон);
    Запрос.УстановитьПараметр("НомерПриказаШаблонКомментария", НомерПриказаШаблонКомментария);
    Запрос.УстановитьПараметр("ГодПриказа", НачалоГода(СтруктураДанныхУниверситет.Дата));

    Запрос.УстановитьПараметр("НомерПриказаБГУ", ДополнительныеПараметрыОтбора.НомерПриказаБГУ);
    Запрос.УстановитьПараметр("Факультет", ДополнительныеПараметрыОтбора.Факультет);
    Запрос.УстановитьПараметр("ВидПриказа", ДополнительныеПараметрыОтбора.ВидПриказа);
    Запрос.УстановитьПараметр("Организация", ДополнительныеПараметрыОтбора.Организация);

    УсловияФильтраСсылок = "ИСТИНА";
    ИспользуетсяОтборПоФильтруСсылок = (ФильтрСсылок.Количество() > 0);
    Если ИспользуетсяОтборПоФильтруСсылок = Истина Тогда
        УсловияФильтраСсылок = СтрШаблон("%1 И ПриказыОбразование.Ссылка В (%2)", УсловияФильтраСсылок, ФильтрСсылок);
    КонецЕсли;
    Запрос.УстановитьПараметр("ИспользуетсяОтборПоФильтруСсылок", ИспользуетсяОтборПоФильтруСсылок);
    Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловияФильтраСсылок", УсловияФильтраСсылок);
    ...
    Если ТаблицаДанных.Количество() > 0 Тогда
        РезультатФункции = Новый Структура(
                "Приказ, Дата, Номер, Комментарий, УникальныйИдентификатор, УстановленаСвязьПоИдентификатору");
        ЗаполнитьЗначенияСвойств(РезультатФункции, ТаблицаДанных[0]);
        РезультатФункции.УстановленаСвязьПоИдентификатору = ИспользуетсяОтборПоФильтруСсылок;
    КонецЕсли;
...
//      * ИдентификаторВнешнийДополнительный - Строка
Функция ПолучитьАктуальныеДанныеИдентификаторовДляВидаОбъектаОбмена(
    ...
    КлючИдентификаторВнешний = "ИдентификаторВнешний";
    КлючИдентификаторВнешнийДополнительный = "ИдентификаторВнешний" + "Дополнительный";

    // Проверка параметров отбора доп. идентификаторов
    Если ВидОбъектаОбменаДополнительный = ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.ПустаяСсылка()
        И ПараметрыОтбора.Свойство(КлючИдентификаторВнешнийДополнительный)
        И НЕ ПустаяСтрока(ПараметрыОтбора[КлючИдентификаторВнешнийДополнительный]) Тогда

        ВызватьИсключение("Несоответствие параметров отбора дополнительного идентификатора");
    КонецЕсли;
    ...
    Если ПараметрыОтбора.Свойство(КлючИдентификаторВнешний) Тогда
        ДополнительныеПараметрыОтбора = СтрШаблон("%1 И %2.ИдентификаторВнешний = &ИдентификаторВнешний",
                ДополнительныеПараметрыОтбора,
                НаименованиеТалицы);
        Запрос.УстановитьПараметр(КлючИдентификаторВнешний, ПараметрыОтбора[КлючИдентификаторВнешний]);
    КонецЕсли;
    Если ПараметрыОтбора.Свойство(КлючИдентификаторВнешнийДополнительный)
        И ВидОбъектаОбменаДополнительный <> ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.ПустаяСсылка() Тогда
        ДополнительныеПараметрыОтбора = СтрШаблон("%1 И %2.ИдентификаторВнешнийДополнительный = &ИдентификаторВнешнийДополнительный",
                ДополнительныеПараметрыОтбора,
                НаименованиеТалицы);
        Запрос.УстановитьПараметр(КлючИдентификаторВнешнийДополнительный, ПараметрыОтбора[КлючИдентификаторВнешнийДополнительный]);
    КонецЕсли;
...
//  ВидОбъектаОбменаДополнительный - ПланВидовХарактеристикСсылка.ГП_ТипыОбъектовОбменаУниверситетБГУ, Неопределено
//  ИдентификаторВнешнийДополнительный - Строка, Неопределено
// Возвращаемое значение:
//  - Булево
Функция ЗаписатьСоответствияИдентификаторовОбмена(
        Знач Идентификатор, Знач ИдентификаторВнешний, Знач ВидОбъектаОбмена,
        Знач ИдентификаторТранзакцииСоздания, Знач ДатаСоздания = Неопределено,
        Знач ВидОбъектаОбменаДополнительный = Неопределено, Знач ИдентификаторВнешнийДополнительный = Неопределено) Экспорт

    ВидОбъектаОбменаДополнительный = ?(ВидОбъектаОбменаДополнительный = Неопределено,
            ПланыВидовХарактеристик.ГП_ТипыОбъектовОбменаУниверситетБГУ.ПустаяСсылка(), ВидОбъектаОбменаДополнительный);
    Если ИдентификаторВнешнийДополнительный = Неопределено ИЛИ ПустаяСтрока(ИдентификаторВнешнийДополнительный) Тогда
        ИдентификаторВнешнийДополнительный = "";
    КонецЕсли;
    ...
    НаборЗаписей.Отбор.ИдентификаторВнешнийДополнительный.Установить(ИдентификаторВнешнийДополнительный);
    ...
    НоваяЗапись.ИдентификаторВнешнийДополнительный = ИдентификаторВнешнийДополнительный;

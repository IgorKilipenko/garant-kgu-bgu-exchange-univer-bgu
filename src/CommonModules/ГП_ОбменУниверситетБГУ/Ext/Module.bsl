...
Функция НовыйПараметрыОтбораСообщенийОтветаУниверситета(Знач ИдентификаторСообщенияЗапроса = Неопределено) Экспорт
    ПараметрыОтбора = ГП_СервисыИнтеграции(ИдентификаторСообщенияЗапроса);
    ПараметрыОтбора.КодПолучателя = ПолучитьКодБазыБГУ();
    ПараметрыОтбора.КодОтправителя = ПолучитьКодБазыУниверситет();
...
//      * Успех - Булево
//      * ТекстСообщения - Строка, Неопределено
Функция ПроверитьСоединениеСБазойУниверситет(Знач ПараметрыВыполнения = Неопределено, Знач АдресРезультата = Неопределено) Экспорт
    ...
    Иначе
        ...
        СтруктураСообщения.Параметры.ТипСообщения = "ПроверкаСоединения";
        ...
        ИначеЕсли РезультатОтправкиСообщения.ИдентификаторыОтправленныхСообщений.Количество() > 0 Тогда
            ...
            Если МассивСообщенийОтвета.Количество() = 0 Тогда
                РезультатФункции.Успех = Ложь;
                РезультатФункции.ТекстСообщения = РезультатФункции.ТекстСообщения = СтрШаблон("Соединение не установлено");
            КонецЕсли;
        КонецЕсли;
    КонецЕсли;
...
Функция НовыйПараметрыОтбораСообщенийОтветаБухгалтерии(Знач ИдентификаторСообщенияЗапроса = Неопределено) Экспорт
    ПараметрыОтбора = ГП_СервисыИнтеграции(ИдентификаторСообщенияЗапроса);
    ПараметрыОтбора.КодПолучателя = ПолучитьКодБазыУниверситет();
    ПараметрыОтбора.КодОтправителя = ПолучитьКодБазыБГУ();

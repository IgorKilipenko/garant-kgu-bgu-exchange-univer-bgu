...
//      * Успех - Булево
//      * ТекстСообщения - Строка, Неопределено
Функция ПроверитьСоединениеСБазойУниверситет(Знач РаботаВТестовомРежиме) Экспорт
...
// Параметры:
//  ПараметрыОтбораПриказов - Структура
//      * ДатаНачала - Дата
//      * ДатаОкончания - Дата
//      * НомерПриказа - Строка
//      * ТолькоПлатнаяОснова - Булево
//  РаботаВТестовомРежиме - Булево
//  ПараметрыВыполнения - Структура
//      * ЗадержкаОпросаКаналаМсек - Число - Опционально, по умолчанию 5000 мс
// Возвращаемое значение:
//  - Структура
Функция ПолучитьДанныеПриказовУниверситета(
        Знач ПараметрыОтбораПриказов, Знач РаботаВТестовомРежиме, Знач ПараметрыВыполнения = Неопределено) Экспорт

    РезультатФункции = Новый Структура("Успех, МассивСообщений, ИдентификаторСообщенияЗапроса, ТекстСообщения",
            Истина, Новый Массив);

    ПараметрыВыполнения = ?(ПараметрыВыполнения = Неопределено, Новый Структура, ПараметрыВыполнения);
    ЗадержкаОпросаКаналаМсек = ?(ПараметрыВыполнения.Свойство("ЗадержкаОпросаКаналаМсек"),
            ПараметрыВыполнения.ЗадержкаОпросаКаналаМсек, 5000);
    ...
    Если НЕ РезультатПроверкиСервиса.Успех Тогда
        ...
        Возврат РезультатФункции; // Возврат. Ошибка доступности сервиса
    КонецЕсли;

    // Формирование сообщения
    СтруктураСообщения = Новый Структура("Параметры", ГП_СервисыИнтеграции.НовыйСтруктураПараметровСообщенияСервисаИнтеграции());
    СтруктураСообщения.Параметры.Вставить("ТипСообщения", "ЗапросДанныхПриказа");
    СтруктураСообщения.Параметры.Вставить("Пользователь", Строка(Пользователи.ТекущийПользователь()));

    // Параметры отбора приказов
    СтруктураСообщения.Параметры.Вставить("ДатаНачала",
        ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуДатыJSON(ПараметрыОтбораПриказов.ДатаНачала));
    СтруктураСообщения.Параметры.Вставить("ДатаОкончания",
        ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуДатыJSON(ПараметрыОтбораПриказов.ДатаОкончания));
    СтруктураСообщения.Параметры.Вставить("НомерПриказа", ПараметрыОтбораПриказов.НомерПриказа);
    СтруктураСообщения.Параметры.Вставить("ТолькоПлатнаяОснова",
        ГП_ОбменУниверситетБГУКлиентСервер.ПолучитьСтрокуИзБулево(ПараметрыОтбораПриказов.ТолькоПлатнаяОснова));

    СообщениеОбмена = ГП_ОбменУниверситетБГУ.НовыйСообщениеВУниверситет(СтруктураСообщения, СервисОбмена);
    РезультатФункции.ИдентификаторСообщенияЗапроса = СообщениеОбмена.Идентификатор;
    ...
    Если РезультатОтправкиСообщения.Успех = Ложь Тогда
        ...
        Возврат РезультатФункции; // Возврат. Ошибка отправки
    КонецЕсли;

    Если РезультатОтправкиСообщения.ИдентификаторыОтправленныхСообщений.Количество() = 0 Тогда
        РезультатФункции.Успех = Ложь;
        РезультатФункции.ТекстСообщения = "Нет сообщений для отправки";

        Возврат РезультатФункции; // Возврат. Нет сообщений для отправки
    КонецЕсли;

    // Получение ответа
    РезультатФункции.ИдентификаторСообщенияЗапроса = РезультатОтправкиСообщения.ИдентификаторыОтправленныхСообщений[0];
    ПараметрыОтбора = НовыйПараметрыОтбораСообщенийОтветаУниверситета(РезультатФункции.ИдентификаторСообщенияЗапроса);
    РезультатФункции.МассивСообщений = ГП_ОбменУниверситетБГУ.ПолучитьСообщенияСервиса(
            КаналОбменаИзУниверситета, ПараметрыОтбора, ЗадержкаОпросаКаналаМсек);

    Если РезультатФункции.МассивСообщений.Количество() = 0 Тогда
        РезультатФункции.Успех = Ложь;
        РезультатФункции.ТекстСообщения = СтрШаблон("Нет ответа от базы << Университет >>");

        Возврат РезультатФункции; // Возврат. Нет ответа
    КонецЕсли;

    // Подготовка и формирование данных
    Для Каждого ТекущееСообщение Из РезультатФункции.МассивСообщений Цикл
        ...
        ТекущееСообщение.Вставить("Данные",
            ГП_ОбменУниверситетБГУ.ПолучитьЗначениеТелаСообщенияУниверситетНаСервере(
                ТекущееСообщение.ПредставлениеТелаСообщения));
    КонецЦикла;

    Возврат РезультатФункции;
КонецФункции

#КонецОбласти // ОбменДанными
